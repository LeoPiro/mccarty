      ...
      <div className="flex items-center justify-center gap-3 mt-8">
        {reviews.map((_, i) => (
          <button
            ...
            onClick={() => setIndex(i)}
            className={`h-2.5 w-2.5 rounded-full transition-colors ${i === index ? "bg-mccarty-dark" : "bg-mccarty-light"}`}
          />
        ))}
      </div>
      <div className="flex justify-center gap-4 mt-6">
        <button onClick={() => setIndex(i => (i - 1 + reviews.length) % reviews.length)} className="text-xs underline text-mccarty-dark hover:text-mccarty-medium transition-colors">
          Prev
        </button>
        <button onClick={() => setIndex(i => (i + 1) % reviews.length)} className="text-xs underline text-mccarty-dark hover:text-mccarty-medium transition-colors">
          Next
        </button>
      </div>
    </div>
